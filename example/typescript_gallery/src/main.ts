...
const modalEditor = document.getElementById('modal-editor') as HTMLTextAreaElement;
const modalTitle = document.getElementById('modal-title')!;
const modalError = document.getElementById('modal-error')!;
const modalTg = new TexGen({ canvas: modalCanvas, width: 512, height: 512 });

let activeDemoLoop: number | null = null;

const createCard = (title: string, badge: string, description: string, initialCode: string, onUpdate?: (code: string) => void) => {
    ...
    card.onclick = () => {
        ...
        if (activeDemoLoop) cancelAnimationFrame(activeDemoLoop);
        ...
        modalEditor.oninput = () => {
            ...
            try {
                if (modalTg.init(modalEditor.value)) {
                    if (onUpdate) onUpdate(modalEditor.value);
                } else {
